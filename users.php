...
<div class="container">
    ...
    <table class="table table-striped main-table">
        <thead><tr><th>#</th><th>Username</th><th>Role</th><th>Gym</th><th>Actions</th></tr></thead>
        <tbody id="userList">
...
$(function(){
    ...
    $(document).on('click', '.btn-edit', function(){
        ...
        const id = tr.data('id');
        $('#user_id').val(id);
        $('#user_username').val(tr.children().eq(1).text().trim());
        $('#user_role').val(tr.children().eq(2).text().trim());
        $('#user_gym').val(tr.children().eq(3).text().trim() || '');
        $('#user_password').val('');
        bsModal.show();
    });
    ...
    $(document).on('click', '.btn-delete', function(){
        ...
        Swal.fire({title:'Delete user?', text:tr.find('td').eq(1).text(), icon:'warning', showCancelButton:true}).then(r=>{ if(r.isConfirmed){ $.post('delete_user.php',{id:id}, function(resp){ if(resp && resp.ok) location.reload(); else Swal.fire('Error', resp.error||'Unable to delete','error'); },'json'); } });
    });
});
</script>
</body>
